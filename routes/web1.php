...
class ProvinciaController extends Controller
{
        ...
        if ($request) {
            ...
            switch (Auth::user()->stringRol->nombre) {
                case 'admin':
                    ...
                    $edit=false;
                    return view('admin.provincia.index', ["provincias" => $provincias, "searchText" => $query, "delegados" => $delegados, "admPro" => $admPro, "menu" => $menu,"edit" => $edit]);
                    break;
                case 'adminProvincia':
                    $query      = trim($request->get('searchText'));
                    $provincias = DB::table('provincias')
                    ...
                    $edit=false;
                    return view("adminProvincia.provincia.index", ["provincias" => $provincias, "searchText" => $query, "menu" => $menu,"edit" => $edit]);
                    break;
            }
        }
    }
    ...
    public function      search(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            if ($provincias) {
                $menu   = $this->MenuIzquierdo(1);
                $edit=false;
                $salida = view('admin.provincia.includes.tablaProvincias', compact('provincias', 'searchText', 'menu','edit'))->render();
    ...
    public function create()
    {
        ...
        $menu = $this->MenuIzquierdo(1);
        $edit=false;
        return view("admin.provincia.create", ["usuarios" => $usuarios, "menu" => $menu,"edit" => $edit]);
    }
    ...
    public function edit($id)
    {
        ...
        $menu = $this->MenuIzquierdo(1);
        $edit=false;
        return view("admin.provincia.editProvincia.edit", ["poblaciones" => $poblaciones, "provincia" => $provincia, "delegados" => $delegados, "delegado" => $delegado, "admPros" => $admPros, "admPro" => $admPro, "menu" => $menu,"edit" => $edit]);
